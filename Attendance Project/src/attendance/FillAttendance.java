...
public class FillAttendance implements ActionListener {
	...
	private void initialize(String section) {
		...
		resetBtn.setIcon(new ImageIcon(FillAttendance.class.getResource("/resources/reset.png")));
...
//					return;
		...
		exitBtn.setIcon(new ImageIcon(FillAttendance.class.getResource("/resources/exit.png")));
		...
		for (int i = 0; i < btnArray.length; i++) {
			if (btn.equals(btnArray[i]))
				if (btn.getActionCommand().equals("no")) {
					present++;
					btnArray[i].setIcon(new ImageIcon(FillAttendance.class.getResource("/resources/right.png")));
					btnArray[i].setActionCommand("yes");
					presentLbl.setText("PRESENT : " + present);
					absentLbl.setText("ABSENT  : " + (total - present));
				} else {
					present--;
					btnArray[i].setIcon(new ImageIcon(FillAttendance.class.getResource("/resources/wrong.png")));
					btnArray[i].setActionCommand("no");
					presentLbl.setText("PRESENT : " + present);
					absentLbl.setText("ABSENT  : " + (total - present));
				}
